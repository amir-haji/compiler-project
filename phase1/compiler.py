...
while True:
    ...
    if token.token_type == T_group.ID or token.token_type == T_group.KEYWORD:
    ...
    if token.token_type not in Panic_states:
        token_list.append(token)
    else:
        Utility.write_lexical_errors(token.lexeme, token.token_type, token_line, is_new_line)
        is_new_line = False
    if token_line != token_current_line:
        is_new_line = True
        Utility.write_tokens(token_list, token_line)
        token_line = token_current_line
        token_list = []
    if the_lexer.is_eof:
        break
for symbol in symbol_dict.keys():
    Utility.write_symbol_file(symbol_dict[symbol])
Utility.check_lexical_error()
